...
if(debug_bool){
  opt=list();
  opt$input_files='../data/restricted/pcawg/pcawg_restricted_snv_counts/3433b1f3-59d1-4ac2-9bbf-e0c4042b4ddc'
  opt$cancer_type="Skin-Melanoma.mucosal";
  opt$feature_type="nucleotidesubstitution1"; opt$output="../data/roo/Skin-Melanoma.mucosal_nucleotidesubstitution1_ROO.RDS"
}else{
  option_list = list(
...
for(name_file in fles_cancer_type){
  ...
  if(opt$feature_type == "signatures"){
    ...
    rds_object[[name_file]] = .tmp_roo_obj ## this is a list of 1
    active_sigs[[name_file]] = list(list(), list())
  }
  
}


#### active_sigs should be a list(list(), list())
