...
ggplot(data.frame(num_samples=num_samples[match(all_pvals$ct, names(num_samples))],
                  ...
                  ct=all_pvals$ct), aes(x=num_samples, y=pvalue, label=ct))+
...
ggplot(cbind.data.frame(effect_size=as.numeric(effect_size1nonexo),
                        num_samples=as.numeric(num_samples),
                        minlogpval=-log(as.numeric(pvals_DMnonexo)),
...
ggplot(cbind.data.frame(effect_size=as.numeric(effect_size1all),
                        num_samples=as.numeric(num_samples_all),
...
effect_size2nonexo <- sapply(enough_samples, function(ct){
  .xx <- all_objects_nonexo[[ct]]
  try(sum((normalise_rw(.xx$Y[1:(nrow(.xx$Y)/2),]) - normalise_rw(.xx$Y[(1+nrow(.xx$Y)/2):(nrow(.xx$Y)),]))**2)/(nrow(.xx$Y)/2)*log(mean(rowSums(.xx$Y))))
})
...
# source("../../../CDA_in_Cancer/code/ProjectDifferentialAbundance/ROO_DifferentialAbundance/roo_global_DA_tests/roo_global_DA_helper.R")
...
```{r, signaturesPCAWG_names_sigs,echo=F}
...
for(ct in enough_samples){
  ...
  colnames_nonexo_notsorted_SP[[ct]] <- try(colnames(load_PCAWG(ct = ct, typedata = "signaturesPCAWG",
                                                 path_to_data = "../../data/")$Y))
...
```{r, plot_betas_signaturesPCAWG, echo=F, fig.height=2, warning=FALSE}
logR_notsorted_SP


for(ct in enough_samples){
  grid.arrange(plot_betas(fullRE_M_SP[[ct]], only_slope=T,
                          names_cats=logR_notsorted_SP[[ct]])+ggtitle('fullRE M'),
               plot_betas(fullRE_DMSL_SP[[ct]], only_slope=T,
                          names_cats=logR_notsorted_SP[[ct]])+ggtitle('fullRE DMSL'),
               plot_betas(fullRE_M_nonexo_SP[[ct]], only_slope=T,
                          names_cats=logR_nonexo_notsorted_SP[[ct]])+ggtitle('fullRE M\nnonexo'),
               plot_betas(fullRE_DMSL_nonexo_SP[[ct]], only_slope=T,
