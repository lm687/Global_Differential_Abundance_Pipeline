...
// full ME DM fitting, with alternative parametrisation

#include <TMB.hpp>
#include "functions.hpp"

template<class Type>
Type dirichlet_multinomial_alt_par( vector<Type> Y_row, vector<Type> alpha, int d)
{
...
Type objective_function<Type>::operator() ()
{
  ...
  for(int l=0; l<n; l++){ // Multinomial draws
    ...
    vector<Type> alpha_l = theta.row(l)*1/exp(log_lambda_vec(l));
    nll -= dirichlet_multinomial_alt_par(lth_row, alpha_l, d);
  }
  return nll;
