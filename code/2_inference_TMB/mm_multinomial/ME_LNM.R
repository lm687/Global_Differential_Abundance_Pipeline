...
df_for_ggplot = cbind.data.frame(
  # u=all_u_scatter,
  min_true_beta=sapply(results_inference_more[1,], function(i) min(abs(i$true_beta)) ), ## no
  max_true_beta=sapply(results_inference_more[1,], function(i) max(abs(i$true_beta)) ), ## no
  min_true_u=sapply(results_inference_more[1,], function(i) min(abs(i$true_u)) ), ## no
  max_true_u=sapply(results_inference_more[1,], function(i) max(abs(i$true_u)) ), ## no
  col_true_u=sapply(results_inference_more[1,], function(i) sum(i$true_u**2) ), ## no
                 col3=sapply(results_inference_more[1,], function(i) sum(i$initial_u**2) ), ## no
                 col_true_beta=sapply(results_inference_more[1,], function(i) sum(i$true_beta**2) ), ## no
                 col_u_estimate=sapply(results_inference_more[1,], function(i) sum(i$u_estimate**2) ), ## no
                 col6=sapply(results_inference_more[1,], function(i) sum(i$betas_estimate**2) ), ## no
                 col7=sapply(results_inference_more[1,], function(i) sum(i$initial_beta**2) ) ## no
)
# ggplot(df_for_ggplot, aes(x=u.1, y=u.2, col=col))+geom_point()
hist(df_for_ggplot$col_u_estimate, breaks=60) ## good to separate?

bool_non_recovered = (df_for_ggplot$col_u_estimate < 0.1)
max(df_for_ggplot$col_u_estimate[bool_non_recovered])
min(df_for_ggplot$col_u_estimate[!bool_non_recovered])

for_PCA = t(sapply(1:ncol(results_inference_more), function(i) c(results_inference_more[1,i][[1]]$betas_estimate,
                                                     results_inference_more[1,i][[1]]$u_estimate,
                                                     results_inference_more[1,i][[1]]$true_beta,
                                                     results_inference_more[1,i][[1]]$true_u,
                                                     results_inference_more[1,i][[1]]$initial_beta,
                                                     results_inference_more[1,i][[1]]$initial_u)))

plot((prcomp(for_PCA)$x)[,1:2], col=df_for_ggplot$u.1)
ggplot(cbind.data.frame(pca=prcomp(for_PCA)$x[,1:2],
                        # cols=df_for_ggplot$col_u_estimate,
                        cols=bool_non_recovered),
                        # cols=df_for_ggplot$u.1),
       aes(x=pca.PC1, y=pca.PC2, col=cols))+geom_point() ## no clear separation of the 
## recovering the thetas and comparing them to the true thetas

par(mfrow=c(3,3))
boxplot(split(x =  df_for_ggplot$col_u_estimate, f = bool_non_recovered)) ## this is the "symptom"
boxplot(split(x =  df_for_ggplot$min_true_u, f = bool_non_recovered))
boxplot(split(x =  df_for_ggplot$col2, f = bool_non_recovered))
boxplot(split(x =  df_for_ggplot$col3, f = bool_non_recovered))
boxplot(split(x =  df_for_ggplot$col4, f = bool_non_recovered))
boxplot(split(x =  df_for_ggplot$col6, f = bool_non_recovered))
boxplot(split(x =  df_for_ggplot$col7, f = bool_non_recovered))

p=1

par(mfrow=c(1,1))
plot(df_for_ggplot$col_true_u, df_for_ggplot$col_true_beta, col=factor(bool_non_recovered), pch=19)
plot(df_for_ggplot$min_true_u, df_for_ggplot$col_true_beta, col=factor(bool_non_recovered), pch=19)
plot(df_for_ggplot$max_true_u, df_for_ggplot$min_true_u, col=factor(bool_non_recovered), pch=19)
plot(df_for_ggplot$max_true_beta, df_for_ggplot$min_true_beta, col=factor(bool_non_recovered), pch=19)
...
for(i in 1:dim(results_inference_more)[2]){
image(t(results_inference_more[1,i][[1]]$true_theta), main='true')
image(t(softmax(cbind(t(results_inference_more[1,i][[1]]$z) %*% replicate(results_inference_more[1,i][[1]]$u_estimate, n = length(results_inference_more[1,i][[1]]$betas_estimate)) +
                      t(results_inference_more[1,i][[1]]$x) %*% matrix(results_inference_more[1,i][[1]]$betas_estimate,nrow=p),
                      0))), main='est')
}
