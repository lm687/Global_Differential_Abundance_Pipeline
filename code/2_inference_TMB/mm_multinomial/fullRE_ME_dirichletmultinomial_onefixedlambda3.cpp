// same as fullRE_ME_dirichletmultinomial.cpp
// but now only one of the two overdispersion parameters remains to be estimated,
// the other one is fixed
// this is to avoid unidentifiability
// fince versions 1 and 2 gave very bad results, I now make the first lambda be the same as the second lambda + 1
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(log_lambda); // log of the parameter for overdispersion in Dirichlet-Multinomial model (1 value, for the second group. The first group has a fixed overdispersion lambda of 0)
  int d_min1 = d - 1;

  // for covariance matrix for random effects
  using namespace density;

  vector<Type> log_lambdas(2);
  log_lambdas(1) = log_lambda;
  log_lambdas(0) = Type(1)+log_lambda;
  ...
  log_lambda_vec = lambda_accessory_mat * log_lambdas; // get a vector of lambdas of length 2*n
