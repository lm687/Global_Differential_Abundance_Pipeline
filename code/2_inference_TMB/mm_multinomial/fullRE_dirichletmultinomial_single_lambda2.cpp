...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(log_lambda); // log of the parameter for overdispersion in Dirichlet-Multinomial model (2 values, one for each group)
  int d_min1 = d - 1;
  ...
  vector<Type> log_lambda_double(2);
  log_lambda_double.fill(log_lambda);
  log_lambda_vec = lambda_accessory_mat * log_lambda_double; // get a vector of lambdas of length 2*n
