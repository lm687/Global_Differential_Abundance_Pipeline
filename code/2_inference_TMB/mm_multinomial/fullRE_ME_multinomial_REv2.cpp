...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(u_large1); // coefficients for the random effects. Even though it is defined as matrix (for TMB matrix multiplication), it is a vector
  PARAMETER_VECTOR(u_large2); // coefficients for the random effects. Even though it is defined as matrix (for TMB matrix multiplication), it is a vector
  PARAMETER_VECTOR(u_large3); // coefficients for the random effects. Even though it is defined as matrix (for TMB matrix multiplication), it is a vector
  PARAMETER_VECTOR(u_large4); // coefficients for the random effects. Even though it is defined as matrix (for TMB matrix multiplication), it is a vector
  int d = Y.cols();
  int n = Y.rows();
  int d_min1 = d - 1;
  ...
  matrix<Type> u_large(num_individuals,d_min1);
  UNSTRUCTURED_CORR_t<Type> nll_mvn(cov_par_RE);

  for(int i=0;i<num_individuals;i++){ 
    u_large(i,0) = u_large1(i);
    u_large(i,1) = u_large2(i);
    u_large(i,2) = u_large3(i);
    u_large(i,3) = u_large4(i);
  }
