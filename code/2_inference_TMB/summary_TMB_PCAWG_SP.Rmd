...
diagRE_DMDL_wSBS1SBS5nonexo_SP_betas_SBS1SBS5 <- do.call('rbind', diagRE_DMDL_wSBS1SBS5nonexo_SP_betas) %>%
  dplyr::filter(type_beta == 'Slope') %>%
  dplyr::select(c('Estimate', 'LogR'))
diagRE_DMDL_wSBS1SBS5nonexo_SP_betas_SBS1SBS5$ct <- gsub("\\.beta.*", "", rownames(diagRE_DMDL_wSBS1SBS5nonexo_SP_betas_SBS1SBS5))

diagRE_DMDL_wSBS1SBS5nonexo_SP_betas_SBS1SBS5 <- dcast(diagRE_DMDL_wSBS1SBS5nonexo_SP_betas_SBS1SBS5, LogR~ct, value.var = 'Estimate')

diagRE_DMDL_wSBS1SBS5nonexo_SP_betas_SBS1SBS5 <- data.frame(t(diagRE_DMDL_wSBS1SBS5nonexo_SP_betas_SBS1SBS5[c(1,5),-1]))
...
```{r, simulation_from_estimates}
ct <- "Bone-Osteosarc"
.sim <- give_sim_from_estimates(tmb_object = fullRE_DMSL_nonexo_SP[[ct]], ct=ct, typedata="signatures",
                                              sigs_to_remove=unique(nonexogenous$V1),
                                              bool_give_PCA = T, path_to_data= "../../data/",
                                            obj_data=all_objects_nonexo_SP[[ct]],
                                              bool_nonexo=T,
                                              model="fullRE_DM", nrow_pca_plot=1)
.sim[[2]]+ggtitle(paste0('Simulation of ', ct))
