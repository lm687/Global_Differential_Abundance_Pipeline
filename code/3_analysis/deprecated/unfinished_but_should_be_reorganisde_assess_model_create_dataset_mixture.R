...
## Here, rather than simulating from the model, I create two populations of counts
## and mix them with known proportions
## This is based on Holmes' Rarefying paper.
...
#   beta[2,] = rgamma(n = d-1, shape = beta_gamma_shape, rate = beta_gamma_shape) ## for the coefficients
# }else if(opt$simulation_generation == 'GenerationC'){
#   beta = matrix(0, nrow=2, ncol=d-1)
#   beta[1,] = runif(n = d-1, min = -1, max = 1)
...
#   }
# }else if(opt$simulation_generation %in% c('GenerationC')){
#   for(l in 1:(2*n)){
#     W[l,] = HMP::Dirichlet.multinomial(Nrs = Nm[l], shape = alpha[l,])
#   }
# }
