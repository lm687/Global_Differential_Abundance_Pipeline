...
# ### difference here below
# Nm_lambdas <- c(rpois(n = n, lambda = Nm_lambda), rpois(n = n, lambda = Nm_lambda/2))
# 
# ct1 <- load_PCAWG("Bone-Osteosarc", typedata = "signaturesPCAWG", simulation = F, path_to_data="../data/")
# 
...
#   rmultinom(1, Nm_lambdas[id_patient], normalise_rw(ct1[sample_ids[id_patient],]))
# })),
# t(sapply(c(1:n), function(id_patient){
#   rmultinom(1, round(Nm_lambdas[id_patient+n]*props_mix1[1]), normalise_rw(ct1[sample_ids[id_patient],]))+
#     rmultinom(1, round(Nm_lambdas[id_patient+n]*props_mix1[2]), normalise_rw(ct2[sample_ids[id_patient],]))
# })))
# 
