##' Changing single lambda, simulating from poisson
rm(list = ls())
...
sd_RE = runif(n = d, min = 0, max = 1) ## standard deviation for random effects
...
if(sim_beta_1)   beta[1,] = runif(n = d, min = -1, max = 1)
...
if(sim_beta_2) beta[2,] = c(beta_gamma_shape, rep(0,d-1))
  ...
  if(is.na(opt$sdRE_input)){
    sim_sd_RE = T}else{
      if(opt$sdRE_input == 'NA'){
        ...
        u <- mvtnorm::rmvnorm(n, mean = rep(0, d), sigma = cov)
      }
    }
}
...
if(sim_sd_RE){
  ## independent random effect
  ## simulate data to get covariances
  aaa <- sapply(1:(d-1), function(i) runif(n = 50))
  aaa <- cbind(aaa, aaa[,1]+runif(n = 50, min = -0.1, 0.1))
  cov <- cov(aaa)
  scale_RE <- 2
  cov <- diag(rep(scale_RE, d)) %*% cov ## scale
  u <- mvtnorm::rmvnorm(n, mean = rep(0, d), sigma = cov)
}else{
...
W <- apply(alpha*100, 2, function(i) sapply(i, rpois, n=1))
