...
## datasets without random effects
## Same as InoRE, but now the standard deviation of beta slope is proportional to gamma_shape
...
if(sim_beta_2){
  if(beta_gamma_shape == 0){
    ...
    beta[2,] = rnorm(n = d-1, mean = beta_gamma_shape, sd = beta_gamma_shape) ## for the slope coefficients
  }
}
...
## create alpha
alphabar = softmax( cbind(t(X_sim)%*%beta, 0) )
