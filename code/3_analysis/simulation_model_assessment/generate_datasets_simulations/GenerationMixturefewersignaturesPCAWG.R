...
## using data from PCAWG
## no random effects, i.e. choosing rows at random
...
ct1 <- load_PCAWG("Liver-HCC", typedata = "signaturesPCAWG", simulation = F, path_to_data="../data/")
ct2 <- load_PCAWG("Eso-AdenoCA", typedata = "signaturesPCAWG", simulation = F, path_to_data="../data/")

selected_sigs <- colnames(ct1$Y)[colnames(ct1$Y) %in% colnames(ct2$Y)]
ct1 <- give_subset_sigs_TMBobj(ct1, colnames(ct1$Y)[!(colnames(ct1$Y) %in% selected_sigs)])
ct2 <- give_subset_sigs_TMBobj(ct2, colnames(ct2$Y)[!(colnames(ct2$Y) %in% selected_sigs)])


## select only the four most prevalent signatures
selected_sigs <- names(sort(colSums(ct1$Y)+colSums(ct2$Y), decreasing = T)[1:4])
ct1 <- give_subset_sigs_TMBobj(ct1, colnames(ct1$Y)[!(colnames(ct1$Y) %in% selected_sigs)])
ct2 <- give_subset_sigs_TMBobj(ct2, colnames(ct2$Y)[!(colnames(ct2$Y) %in% selected_sigs)])


length(colnames(ct1$Y))
colnames(ct2$Y)
...
W <- rbind(t(sapply(Nm_lambdas[1:n], function(sample_mut_toll){
  rmultinom(1, sample_mut_toll, normalise_rw(ct1$Y[sample(1:(nrow(ct1$Y)), 1),]))
})),
t(sapply(Nm_lambdas[(n+1):(2*n)], function(sample_mut_toll){
  rmultinom(1, round(sample_mut_toll*props_mix1[1]), normalise_rw(ct1$Y[sample(1:(nrow(ct1$Y)), 1),]))+
    rmultinom(1, round(sample_mut_toll*props_mix1[2]), normalise_rw(ct2$Y[sample(1:(nrow(ct2$Y)), 1),]))
})))
...
saveRDS(list(objects_counts=objects_counts, d=d, n= n, beta_gamma_shape=beta_gamma_shape, sd_RE=sd_RE, lambda=lambda, Nm_lambda=Nm_lambda,
             X_sim = X_sim, beta = beta, Z_sim = Z_sim, u = u, lambdas = lambdas, alphabar = alphabar, alpha = alpha, Nm = Nm_lambdas, W = W),
        file = opt$outfile)
