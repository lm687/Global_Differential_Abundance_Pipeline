...
lambda_per_sig <- outer(colSums(W[1:(n),])/1000, c(3,2), function(i, j) i*j)
lambda_per_sample <- rbind(t(lambda_per_sig[,rep(1,n)]), t(lambda_per_sig[,rep(2,n)]))

W <- outer(1:nrow(W), 1:ncol(W), Vectorize(function(i,j){
  W[i,j] + rpois(1, lambda_per_sample[i,j])
}))
