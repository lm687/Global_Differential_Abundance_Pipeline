...
ct_name <- "Skin-Melanoma.cutaneous"
...
# > sum(HMP_estimates[[1]]$gamma)
# [1] 11.17396
# > sum(HMP_estimates[[1]]$theta)
# [1] 0.08214253
# > 1/sum(HMP_estimates[[1]]$theta)
# [1] 12.17396
theta1 <- 1/sum(HMP_estimates[[1]]$theta)-1
theta2 <- 1/sum(HMP_estimates[[2]]$theta)-1

W <- rbind(t(sapply(1:n, function(id_patient){
  HMP::Dirichlet.multinomial(Nrs = sum(ct1[sample_ids[id_patient],]), shape = theta1*normalise_rw(ct1[sample_ids[id_patient],]))
})),
t(sapply(c(1:n), function(id_patient){
  HMP::Dirichlet.multinomial(Nrs = round(sum(ct2[sample_ids[id_patient],])*props_mix1[1]),
                             shape = theta2*normalise_rw(ct1[sample_ids[id_patient],]))+
    HMP::Dirichlet.multinomial(Nrs = round(sum(ct2[sample_ids[id_patient],])*props_mix1[2]),
                               theta2*normalise_rw(ct2[sample_ids[id_patient],]))
})))
