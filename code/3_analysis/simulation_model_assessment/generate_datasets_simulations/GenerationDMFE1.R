...
## Fixed effects Dirichlet-multinomial
...
option_list = list(
  ...
  make_option(c("--sdRE_input"), type="character", default=NA,                ## not used
              help="Fixed standard deviations for RE", metavar="character"),  ## not used
...
sd_RE = NA ## random effects (not applicable)
Z_sim = NA ## random effects (not applicable)
u = NA     ## random effects (not applicable)
...
saveRDS(list(objects_counts=objects_counts, d=d, n= n, beta_gamma_shape=beta_gamma_shape, sd_RE=sd_RE, lambda=opt$lambda, Nm_lambda=Nm_lambda,
