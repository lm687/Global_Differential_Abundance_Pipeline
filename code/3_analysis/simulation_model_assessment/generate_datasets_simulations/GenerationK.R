## modifying the probabilities underlying a multinomial distribution with gaussian noise
...
shifts_sigs <- rgamma(n = d, rate=beta_gamma_shape, shape =beta_gamma_shape)
shifts_samples <- matrix(sapply(shifts_sigs, function(mean_shift){
  sapply(rnorm(n = n, mean = mean_shift/3, sd = 0.3), function(i) max(c(0,i)))
  }), ncol=d)
