...
varct1 <- apply(normalise_rw(ct1), 2, var)*colSums(ct1)/20
varct2 <- apply(normalise_rw(ct2), 2, var)*colSums(ct2)/20

W <- rbind(round(W[1:(n),]+sapply(1:d, function(j) rnorm((n), mean = 0, sd = varct1))),
           round(W[(1+n):(2*n),]+sapply(1:d, function(j) rnorm((n), mean = 0, sd = varct2))))
W[W<0] <- 0
