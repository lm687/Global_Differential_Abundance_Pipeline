...
for i in ['../data/roo/'+x for x in os.listdir("../data/roo/") if 'ROO' in x]:
    script = "~/.conda/envs/rstan_env/bin/Rscript --vanilla 2_inference/fit_PCAWG.R --cancertype {} --typedata {} --infile {} --output {} --iterations {} --model DM".format(i.split('_')[0], (i.split('_')[1]), i, i.replace("roo/", "inference/").replace("_ROO", "_"+str(nits)+'_'+'DM'+"ROO").replace(".RDS", ".RData"), str(nits))
