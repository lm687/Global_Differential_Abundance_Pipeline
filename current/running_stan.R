...
opt=list()
opt$infile = "../data/roo/ColoRect-AdenoCA_signatures_ROO.RDS"
opt$output = "~/Desktop/stan_output"
opt$niterations = 2000
opt$model = "M_fullME"

fle <- opt$infile
objects_sigs_per_CT_features <- load_PCAWG(ct = "ColoRect-AdenoCA", typedata = "signatures", simulation = F, path_to_data = "../data/")
...
fit_stan <- stan(file = model_file_name, data = stan_data, chains = 1)

fit_stan_subset = extract(fit_stan, pars = names(fit_stan)[!grepl('ularge', names(fit_stan))])

png("~/Desktop/pairs_ColoRect-AdenoCA_signatures_MfullRE_stan.png",
